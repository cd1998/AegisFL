...
def train_model(dataset,num_clients,num_mal_clients,scaling_factor,local_iteration,global_iteration,attack_type):
    ...
    aggregation_model_vector = torch.sub(parameters_to_vector(aggregation_model.parameters()), global_gradient_vector * 0.25)
    vector_to_parameters(aggregation_model_vector,aggregation_model.parameters())
    torch.save(aggregation_model.state_dict(), f'model/shieldfl.pt')
    aggregation_model.load_state_dict(torch.load(f'model/shieldfl.pt'))
    for i in range(num_clients):
        client_model[i].load_state_dict(torch.load(f'model/shieldfl.pt'))
