...
def train_model(dataset,num_clients,num_mal_clients,scaling_factor,local_iteration,global_iteration,attack_type):
    ...
    torch.save(aggregation_model.state_dict(), f'model/shieldfl_HE.pt')
    aggregation_model.load_state_dict(torch.load(f'model/shieldfl_HE.pt'))
    for i in range(num_clients):
        client_model[i].load_state_dict(torch.load(f'model/shieldfl_HE.pt'))
    ...
    for round in range(global_iteration):
        # random_numbers = np.random.choice(30, num_mal_clients, replace=False)
        random_numbers = np.arange(num_mal_clients)
        print('order',random_numbers)
        for epoch in range(local_iteration):
            for i in range(num_clients):
        ...
        print("L2",matrix_norms)
        dot_products = np.dot(numpy_array, global_gradient_numpy)
        cosine_similarity = dot_products / (matrix_norms * vector_norm)
        print(cosine_similarity)
        min_index = np.argmin(cosine_similarity)
        print(min_index)
        malicious_client = numpy_array[min_index]
        su = []
        for i in range(num_clients):
            weight = 1 - np.dot(malicious_client,numpy_array[i]) / (np.linalg.norm(malicious_client) * np.linalg.norm(numpy_array[i]))
            su.append(weight)
        print('su',su)
        s = sum(su)
        client_gradient = []
        for i in range(num_clients):
            client_gradient.append(client_gradient_round[i] * su[i])

        global_gradient_vector = sum(client_gradient) / s
        global_gradient_detach = global_gradient_vector.detach()
        global_gradient_numpy = global_gradient_detach.numpy()
        aggregation_model_vector = torch.sub(parameters_to_vector(aggregation_model.parameters()),
                                             global_gradient_vector * 0.25)
