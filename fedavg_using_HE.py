...
def train_model(dataset,num_clients,num_mal_clients,scaling_factor,local_iteration,global_iteration,attack_type):
    ...
    if dataset == "HAR":
        ...
        copy.deepcopy(each_worker_data)
        ...
        each_worker_data_du, each_worker_label_du, X_test_du, y_test_du = get_mnist.get_data()
    ...
    cipher_aggre_model_1, cipher_aggre_model_2 = modified_ckks.encrypt(parameters_to_vector(aggregation_model.parameters()).detach().numpy(),model_len)
    cipher_client_model_round_1 = []
    cipher_client_model_round_2 = []
    for i in range(num_clients):
        cipher1, cipher2 = modified_ckks.encrypt(parameters_to_vector(client_model[i].parameters()).detach().numpy(),model_len)
    ...
    torch.save(aggregation_model.state_dict(), f'model/fedavg_HE.pt')
    aggregation_model.load_state_dict(torch.load(f'model/fedavg_HE.pt'))
    for i in range(num_clients):
        client_model[i].load_state_dict(torch.load(f'model/fedavg_HE.pt'))
    ...
    if attack_type == "backdoor":
        ...
        print(each_worker_label_malicious[1])
        print(each_worker_label[1])
        print(each_worker_label_du[1])
    ...
    for round in range(global_iteration):
        ...
        for i in range(num_clients):
            a = torch.sub(parameters_to_vector(aggregation_model.parameters()),
        ...
        cipher_aggre_model_1, cipher_aggre_model_2 = modified_ckks.encrypt(parameters_to_vector(aggregation_model.parameters()).detach().numpy(), model_len)
        cipher_client_gradient_round_1 = []
        cipher_client_gradient_round_2 = []
        for i in range(num_clients):
            cipher1, cipher2 = modified_ckks.encrypt(client_gradient_round[i].detach().numpy(), model_len)
            cipher_client_gradient_round_1.append(cipher1)
            cipher_client_gradient_round_2.append(cipher2)
        ...
        global_gradient_vector = sum(client_gradient_round) / num_clients
        global_gradient_detach = global_gradient_vector.detach()
        global_gradient_numpy = global_gradient_detach.numpy()


        aggregation_model_vector = torch.sub(parameters_to_vector(aggregation_model.parameters()),global_gradient_vector)
...
if __name__ == "__main__":
    train_model(dataset="HAR",num_clients=30,num_mal_clients=12,scaling_factor=5,local_iteration=50,global_iteration=100,attack_type="no")
